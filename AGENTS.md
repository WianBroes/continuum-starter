...
1. Relire son journal et remplir, **en fin de `SESSION.md`**, les sections utiles (titres exacts ; une section absente = rien à transmettre) :
...
4. **Reprise après clôture** (l'utilisateur reparle sans avoir fermé l'agent) : toujours `ouvrir` avant de répondre, jamais réécrire dans le dossier clos. Le script reconnaît le même process et marque la nouvelle session `suite_de: <id>`. À la reclôture, `## Pour BASE` seulement si la suite a changé quelque chose.
