...
3. `bash outils/session.sh clore` : écrit `fin:`, déplace le dossier vers `closed/`, puis lance la fusion (`outils/consolide.sh`, sous verrou) qui traite **tous** les dossiers de `closed/` : copie des sections ci-dessus dans les fichiers communs (sans `## Pour BASE` : aucune entrée, sauf dossier clos a posteriori, orphelin → « Clos sans synthèse »), plafonds de `BASE.md` (§Historique 5 entrées, §Décidé 5 ; surplus → `_archive/`, verbatim), déplacement vers `sealed/`, commit sur ces seuls chemins. Verrou occupé → rien à faire, la fusion suivante prendra tout.
